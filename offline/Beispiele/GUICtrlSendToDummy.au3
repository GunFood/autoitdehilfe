...
Func Example()
	...
	GUICtrlSendToDummy($g_idUserDummy, 1) ; Setzt den Status auf geklickt Onclick
...
Func OnClick()
	Return GUICtrlSendToDummy($g_idUserDummy) ; Sendet eine Nachricht zu dem Dummy-Control, dass der Schließen-Button ausgewählt wurde. Dieses ruft dann die 'OnExit'-Funktion auf.
EndFunc   ;==>OnClick


Func OnDummy()
	If GUICtrlRead($g_idUserDummy) Then
		GUISetBkColor(0x000000FF) ; Ändertdie Hintergrundgartbe der Farbe auf den dummy Status
	Else
		Exit
	EndIf
EndFunc   ;==>OnDummy
