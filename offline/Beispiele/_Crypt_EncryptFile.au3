...
Func Example()
	...
	Local $hGui = GUICreate("Datei Verschlüsseln", 425, 100)
	Local $idInp_Source = GUICtrlCreateInput("", 5, 5, 200, 20)
	Local $idBtn_SourceBrowse = GUICtrlCreateButton("...", 210, 5, 35, 20)

	Local $idInp_Destination = GUICtrlCreateInput("", 5, 30, 200, 20)
	Local $idBtn_DestinationBrowse = GUICtrlCreateButton("...", 210, 30, 35, 20)

	GUICtrlCreateLabel("Passwort:", 5, 60, 200, 20)
	Local $idInp_Password = GUICtrlCreateInput("", 5, 75, 200, 20)
	...
	Local $idBtn_Encrypt = GUICtrlCreateButton("Verschlüsseln", 355, 70, 65, 25)
	...
	While 1
		Switch GUIGetMsg()
			...
			Case $idBtn_SourceBrowse
				$sFilePath = FileOpenDialog("Datei zum entschlüsseln auswählen.", "", "Alle Dateien (*.*)") ; Datei zum Entschlüsseln auswählen.
				If @error Then
					ContinueLoop
				EndIf
				GUICtrlSetData($idInp_Source, $sFilePath) ; Pfad in der Inputbox anzeigen.

			Case $idBtn_DestinationBrowse
				$sFilePath = FileSaveDialog("Datei speichern als ...", "", "Alle Dateien (*.*)") ; Datei auswählen, um die entschlüsselten Daten zu speichern.
				If @error Then
					ContinueLoop
				EndIf
				GUICtrlSetData($idInp_Destination, $sFilePath) ; Pfad in der Inputbox anzeigen.
			...
			Case $idBtn_Encrypt
				$sSourceRead = GUICtrlRead($idInp_Source) ; Quelldateipfad einlesen.
				$sDestinationRead = GUICtrlRead($idInp_Destination) ; Zieldateipfad einlesen.
				$sPasswordRead = GUICtrlRead($idInp_Password) ; Passwort-Input auslesen.
				If StringStripWS($sSourceRead, $STR_STRIPALL) <> "" And StringStripWS($sDestinationRead, $STR_STRIPALL) <> "" And StringStripWS($sPasswordRead, $STR_STRIPALL) <> "" And FileExists($sSourceRead) Then ; Prüfen ob eine Datei und ein Passwort gegeben sind.
					If _Crypt_EncryptFile($sSourceRead, $sDestinationRead, $sPasswordRead, $iAlgorithm) Then ; Verschlüsseln der Datei.
						MsgBox($MB_SYSTEMMODAL, "Erfolgreich", "Operation erfolgreich")
					Else
						Switch @error
							Case 2
