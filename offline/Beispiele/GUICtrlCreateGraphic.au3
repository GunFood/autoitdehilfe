...
Global $g_a_idGrc_Array[$g_MAXGr + 1] ; Die Einträge 0 und $g_MAXGr Einträge werden nicht verwendet, um bei GUICtrlDelete ein Ergebnis zu erhalten
Global $g_idBtn_Del
...
Func Example()
	...
	Do
		...
		If $idMsg = $g_idBtn_Del Then
			GUICtrlDelete($g_a_idGrc_Array[$i])
			$i = $i + $iInc
			If $i < 0 Or $i > $g_MAXGr Then Exit
		EndIf
		If $idMsg > 0 Then MsgBox($MB_SYSTEMMODAL, "geklickt", $idMsg & @CRLF & $g_a_idGrc_Array[5], 2)
...
Func CreateChild()
	...
	$g_a_idGrc_Array[1] = GUICtrlCreateGraphic(20, 50, 100, 100)
	...
	$g_a_idGrc_Array[2] = GUICtrlCreateGraphic(220, 50, 100, 100)
	...
	$g_a_idGrc_Array[5] = GUICtrlCreateGraphic(150, 10, 50, 50, 0)
