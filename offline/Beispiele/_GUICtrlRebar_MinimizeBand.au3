...
Func Example()
	Local $hGui, $idBtn_Exit, $hReBar, $hToolbar, $hCombo, $idInput
	...
	; Fügt eine Gruppe mit dem Toolbar-Control am Anfang der Rebar hinzu
	_GUICtrlRebar_AddToolBarBand($hReBar, $hToolbar)

	; Fügt eine Gruppe mit dem Inputbox-Control hinzu
	_GUICtrlRebar_AddBand($hReBar, GUICtrlGetHandle($idInput), 120, 200, "Name:")

	; Fügt eine Gruppe mit dem Combobox-Control hinzu
	_GUICtrlRebar_AddBand($hReBar, $hCombo, 120, 200, "Verzeichnis " & @WindowsDir & "\*.exe:")
	...
	MsgBox($MB_SYSTEMMODAL, "Information", "Gruppe 1 minimieren")
	_GUICtrlRebar_MinimizeBand($hReBar, 1)
	MsgBox($MB_SYSTEMMODAL, "Information", "Maximiert Gruppe 1 auf die maximale Größe")
	_GUICtrlRebar_MaximizeBand($hReBar, 1, False)
	MsgBox($MB_SYSTEMMODAL, "Information", "Maximiert Gruppe 0 auf die ideale Größe")
