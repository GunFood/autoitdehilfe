...
Func _EventProc($hEventHook, $iEvent, $hWnd, $iObjectID, $iChildID, $iThreadId, $iEventTime)
	...
	Switch $iEvent
		Case $EVENT_SYSTEM_MENUPOPUPSTART
			; Hinzufügen von "View - Calculator"
			$g_hMenu = _SendMessage($hWnd, $MN_GETHMENU)
			If (_GUICtrlMenu_IsMenu($g_hMenu)) And (StringInStr(_GUICtrlMenu_GetItemText($g_hMenu, 0), 'Statusleiste')) And (StringInStr(_WinAPI_GetWindowFileName($hWnd), 'notepad.exe')) Then
				$g_iIndex = _GUICtrlMenu_GetItemCount($g_hMenu)
				_GUICtrlMenu_InsertMenuItem($g_hMenu, $g_iIndex, 'Taschnerechner' & @TAB & ':-)')
