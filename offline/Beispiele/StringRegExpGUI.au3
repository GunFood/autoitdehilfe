...
Func readDatFile()
	...
	Else
		...
		$sDat = StringReplace($sDat, $sHeader, "") ; Header löschen
		; @CRLF löschen
...
Func doPtnDel($x)
	...
	; Wenn $x in der Mitte der Datei war, müssen nun alle doppelten @CRLF gelöscht werden.
	$sDat = StringReplace($sDat, @CRLF & @CRLF, @CRLF)
	; Wenn $x am Anfang / Ende der Datei stand, so müssen nun alle anführenden und folgenden @CRLF gelöscht werden.
	If StringLeft($sDat, 2) = @CRLF Then $sDat = StringTrimLeft($sDat, 2)
	If StringRight($sDat, 2) = @CRLF Then $sDat = StringTrimRight($sDat, 2)
