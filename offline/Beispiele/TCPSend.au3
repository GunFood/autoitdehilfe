...
Func Example()
	TCPStartup() ; Starte den TCP Service.

	; Registriere OnAutoItExit um es aufzurufen, wenn das Script beendet wird.
	OnAutoItExitRegister("OnAutoItExit")

	; Weise lokalen Variablen die loopback IP-Adresse (localhost) und den Port zu.
...
Func MyTCP_Client($sIPAddress, $iPort)
	; Weise einer lokalen Variable den Socket zu und verbindet sich zu einem lauschenden Socket mit der angegebenen IP-Adresse und Port.
	Local $iSocket = TCPConnect($sIPAddress, $iPort)
	Local $iError = 0

	; Wenn ein Fehler aufgetaucht ist, so zeige diesen an und gebe False zurück
	If @error Then
	...
	; Sende den String "tata" zum Server.
...
Func MyTCP_Server($sIPAddress, $iPort)
	; Weise einer lokalen Variable den Socket zu und bindet die angegebene IP-Adresse sowie Port mit einer maximalen Anzahl von 100 anstehenden Verbindungen.
	...
	; Initialisiere eine lokale Variable um sie als Clientsocket zu nutzen.
	Local $iSocket = 0

	Do ; Warte solange bis sich jemand verbindet (Unbegrenzt).
		; Akzeptiere eingehende Verbindungen sofern präsent (Socket wird geschlossen wenn die Aufgabe abgeschlossen wurde; Ein Socket pro Client).
	...
	; Schließe den gebundenen Socket damit er von anderen Anwendungen gebunden werden kann.
	TCPCloseSocket($iListenSocket)

	; Weise einer loaklen Variable die erhaltenen Daten zu.
	...
	; Zeige den empfangenen String an.
...
Func OnAutoItExit()
	TCPShutdown() ; Schließe den TCP Service.
